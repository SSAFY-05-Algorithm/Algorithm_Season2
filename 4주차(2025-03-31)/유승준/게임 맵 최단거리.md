...
## 문제 분석
BFS로 `(1,1)` 부터 4방 탐색을 해준다. 가장 빠른 길을 가기 위해서는 최대한 적은 칸의 개수를 거쳐야 한다. 방문하는 각 칸마다 거쳐간 칸의 개수를 저장시키고, 모든 탐색이 끝나면 `(N, N)`의 값이 -1 즉, 도달 불가능한 경우가 아니라면면 최단경로를 가지는 가장 적은 칸의 개수가 된다.
...
## 느낀점
방문배열을 사용하지 않으므로, 시작점을 제외하고 방문하지 않은 칸의 초기값은 항상 1이라는 점을 이용해서 더 큰 값으로 덮어씌워지지 않도록 했다.
