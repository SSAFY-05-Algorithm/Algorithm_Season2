...
### 아이디어
- 그냥 시뮬 인가?? -> 삭제하고 다음 true 찾을때 시간복잡도가 O(n) * cmd -> 터짐
- 다음 true를 짧은 시간만에 찾아야함 -> treeSet은 자신보다 크거나 작은 값을 구하는게 O(log N) * cmd -> 안터짐
...
### 시간 복잡도
- O(mn logN) 이지만 X의 최대 100만 
- 사실 망한 코드임

### 느낀점
- 방금 시간 복잡도 계산하다 망한 코드라는 것을 깨달음
- 이건 링크드 리스트 구현으로 풀어야 할 듯 이럼 O(m+n)