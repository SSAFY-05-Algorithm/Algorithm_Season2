...
## 문제 분석
모든 사람이 심사를 받는데 최소가 되는 시간을 구하기 위해 시간에 대해서 이분탐색을 실시한다. 각 심사대 별로 중간값을 갖는 시간동안 입국심사를 마치는 사람을 합산하여 그 수가 n보다 큰 경우, 같거나 작은 경우로 나누어 시간을 좁혀나간다.
...
## 느낀점
`right`변수에 최악의 경우를 갖는 시간을 초기화하는 과정에서 `long`형변환을 제대로 해주지 못했다. 값의 범위를 잘 확인해서 그에 맞는 형변환을 해주는 것을 잊지 말자.